...
class GeometricWassersteinScene(Scene):
    def construct(self):
        ...
        # 1. Creative Intro: phase-shifted sine wave animation
        ...
        play_voiceover_and_wait(self, 37, 0)

        self.play(FadeOut(sine_live), FadeOut(title_group), FadeOut(subtitle_text), run_time=TIME_FAST)

        # Subtitle 1
        sub1 = create_bottom_caption("Geometric Wasserstein đưa cấu trúc hình học manifold của dữ liệu vào DRO.")
        self.play(FadeIn(sub1))
        # seg 1: "Geometric Wasserstein đưa manifold geometry vào DRO..."
        play_voiceover_and_wait(self, 37, 1)
        ...
        # seg 2: "đây là manifold dữ liệu, bề mặt mà dữ liệu nằm trên đó..."
        play_voiceover_and_wait(self, 37, 2)

        # Points on manifold (Green) + noisy outlier (Red)
        xs = [-2.5, -1.5, -0.5, 0.5, 1.5, 2.5]
        dots = VGroup(*[Dot(axes.c2p(x, 0.15 * x**2 - 1.2), color=THEME_EMERALD, radius=0.07) for x in xs])
        ...
        # seg 3: "mẫu hợp lệ nằm dọc manifold, mẫu nhiễu bị cô lập..."
        play_voiceover_and_wait(self, 37, 3)
        ...
        # seg 4: "hard samples nằm trong vùng cấu trúc; noisy thì cô lập..."
        play_voiceover_and_wait(self, 37, 4)

        # Standard Wasserstein: direct transport to outlier (Bad!)
        std_arrows = VGroup(
        ...
        # seg 5: "standard Wasserstein vận chuyển thẳng, bị hút vào mẫu nhiễu..."
        play_voiceover_and_wait(self, 37, 5)
        ...
        # Geometric Wasserstein: arrows follow manifold
        geo_arrows = VGroup()
        ...
        # seg 6: "geometric Wasserstein chạy dọc manifold, bỏ qua ngoại lai..."
        play_voiceover_and_wait(self, 37, 6)
        ...
        # seg 7: "độ robust nên tôn trọng hình học dữ liệu..."
        play_voiceover_and_wait(self, 37, 7)
