...
class DeploymentConclusionScene(Scene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            run_time=TIME_NORMAL, rate_func=smooth
        )
        ...
        play_voiceover_and_wait(self, 40, 0)
        self.play(FadeOut(title_group), FadeOut(subtitle_text), run_time=TIME_FAST)
        ...
        # seg 1: "sau deployment, drop do X-shift, Y|X-shift, hoặc cả hai..."
        play_voiceover_and_wait(self, 40, 1)

        # Shared Distribution transition: P -> S -> Q
        ...
        self.play(FadeIn(p_circle), FadeIn(p_label), FadeIn(q_circle), FadeIn(q_label), run_time=TIME_NORMAL)
        # seg 2: "ta có phân phối nguồn P và đích Q..."
        play_voiceover_and_wait(self, 40, 2)
        ...
        self.play(FadeIn(s_circle), FadeIn(s_label), Create(arrow_ps), Create(arrow_sq), run_time=TIME_NORMAL)
        # seg 3: "tập trung vào phần chung S, tách sampling khỏi mechanism..."
        play_voiceover_and_wait(self, 40, 3)
        ...
        # seg 4: "thông điệp cuối: OOD không phải thuật toán đơn lẻ, ERM DRO IRM HRM là mảnh ghép..."
        play_voiceover_and_wait(self, 40, 4)

        # Subtitle 4 + Highlight HRM
        ...
        # seg 5: "nó là workflow nhận biết heterogeneity, không phải model lớn hơn..."
        play_voiceover_and_wait(self, 40, 5)
        ...
        self.play(Write(final_title), FadeIn(final_subtitle), FadeIn(final_glow), run_time=TIME_NORMAL)
        # seg 6: "điểm khởi đầu đúng là thấu hiểu heterogeneity của dữ liệu..."
        play_voiceover_and_wait(self, 40, 6)
        ...
        # seg 7: "hãy thấu hiểu heterogeneity trước khi đối phó shift..."
        play_voiceover_and_wait(self, 40, 7)

        # Clear conclusion, show Thank You
        self.play(
        ...
        thanks_text = Text("Cảm ơn các bạn đã lắng nghe!", font_size=SIZE_TITLE - 6, font=FONT_PRIMARY, weight=BOLD)
        thanks_text.set_color_by_gradient(THEME_EMERALD, THEME_BLUE_LIGHT)
        thanks_glow = create_3b1b_glow(thanks_text, color=THEME_EMERALD, n_layers=5, opacity=0.25)
        thanks_sub = Text("Hẹn gặp lại trong những hành trình khám phá tiếp theo", font_size=SIZE_BODY - 6, color=TEXT_SECONDARY, font=FONT_PRIMARY)
        thanks_group = VGroup(thanks_glow, thanks_text).move_to(UP * 0.4)
        thanks_sub.next_to(thanks_group, DOWN, buff=0.35)

        # Small decorative hearts/dots rising
        deco = VGroup(*[
            Dot(point=[np.random.uniform(-5, 5), np.random.uniform(-3, -1.5), 0],
                color=THEME_EMERALD if np.random.rand() > 0.5 else THEME_BLUE_LIGHT, radius=0.05)
            for _ in range(12)
        ])

        self.play(
            Write(thanks_text), FadeIn(thanks_glow), FadeIn(thanks_sub, shift=UP * 0.2),
            LaggedStart(*[FadeIn(d, shift=UP * 0.5) for d in deco], lag_ratio=0.1),
            run_time=TIME_SLOW
        )
        # seg 8: "cảm ơn các bạn đã lắng nghe..."
        play_voiceover_and_wait(self, 40, 8)

        self.play(
            FadeOut(thanks_group), FadeOut(thanks_sub), FadeOut(deco),
            run_time=TIME_NORMAL
        )
        self.wait(1.0)
