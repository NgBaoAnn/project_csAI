...
class StabilityFeatureScene(Scene):
    ...
    def construct(self):
        ...
        # 1. Creative Intro: Concentric elastic circles and radar scan line sweep (Faded circle colors to make text more readable)
        ...
        # Draw Failure Set boundary (Dashed Red Line - Shifted LEFT * 2.0 to avoid overlapping with right sliders)
        ...
        # Draw Stability Margin Arrow pointing to the new boundary X = -2.0
        ...
        # Animate Slider 1 (Age) moving right, while model_dot moves right (closer to failure set)
        ...
        # Pulsate model_dot red as it gets dangerously close
        self.play(
            model_dot.animate.set_color(THEME_RED).scale(1.25),
            run_time=0.5
        )
        self.play(
            model_dot.animate.scale(1/1.25),
            run_time=0.5
        )
        self.wait(8.0)
