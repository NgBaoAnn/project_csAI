...
class CropYieldScene(Scene):
    def construct(self):
        ...
        # Field box (representing crop field) at the bottom
        ...
        # We place dots column-by-column (left-to-right) so LaggedStart works perfectly
        for col_idx in range(8):
            ...
            for row_idx in range(3):
                ...
                # Determine target color (Corn = Blue, Soy = Amber)
        ...
        # Glowing vertical sweeping line starting at the left edge
        sweeping_line = Line(
            start=[-3.7, 0.5, 0],
            end=[-3.7, -2.1, 0],
            color=THEME_PURPLE,
        ...
        # Title "Crop Yield" positioned cleanly at the top (no overlap)
        ...
        # Run sweep, update dot colors, and show titles simultaneously
        self.play(
            sweeping_group.animate.move_to([3.7, -0.8, 0]),
            LaggedStart(
                *[
                    dot.animate.set_stroke(color=target_color, width=2.5).set_fill(color=target_color, opacity=0.8)
                    for dot, target_color in zip(dots, target_colors)
                ],
                lag_ratio=0.08,
                run_time=3.2
            ),
        ...
        # Fade out sweeping line and wait
        ...
        # Helper to create a grid of circles
        ...
        # Left colors (True crop map) - e.g. corn (blue) vs soy (amber)
        # We make a diagonal split: top-left is blue, bottom-right is amber
        true_colors = []
        for r in range(map_size):
            for c in range(map_size):
                if r + c < map_size:
        ...
        # Right colors (initially all gray, representing hidden status)
        ...
        # Draw a checkmark/alignment highlight
        ...
        self.play(
            ...
            FadeOut(left_label),
            FadeOut(right_label),
            FadeOut(right_colors_target),
            FadeOut(learning_arrow),
            FadeOut(arrow_label),
            FadeOut(glow_right),
            FadeOut(alignment_text),
            FadeOut(insight),
            run_time=TIME_NORMAL
        )
        self.wait(2.5)
